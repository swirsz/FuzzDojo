...
int LLVMFuzzerTestOneInput(const uint8_t *data, size_t dataLen) {
  char *inFileName = malloc(200);
  char *outFileName = malloc(200);
  memset(inFileName, 0, 200);
  memset(outFileName, 0, 200);
  strcpy(inFileName, "/tmp/minigzip_fuzzerXXXXXX");
  close(mkstemp(inFileName));
  snprintf(inFileName, 200, "%s.out", inFileName);
  snprintf(outFileName, 200, "%s.gz", inFileName);
