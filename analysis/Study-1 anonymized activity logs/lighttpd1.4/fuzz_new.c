...
int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size) {
    ...
    // burl_append_encode_nde(psrc, new_str, size);
    buffer *psrc1 = buffer_init();
    // buffer_commit(psrc1, size);
    buffer_copy_string(psrc1, data);
    buffer_string_prepare_append(psrc1, size);
    buffer_append_uint_hex_lc(psrc1, size);
    buffer_append_int(psrc1, size);
    buffer_append_string(psrc1, data);
    // buffer_append_iovec(psrc, data, size);
