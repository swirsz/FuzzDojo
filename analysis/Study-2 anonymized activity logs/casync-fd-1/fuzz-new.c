...
int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size) {
	...
	*/
        ...
        size_t out_size2 = MAX(size, 128u), /* Make the buffer a bit larger for very small data */ ret_done2;
        _cleanup_free_ void *buf2 =  malloc(out_size);
        if (!buf2) {
                log_oom();
                return 0;
        }

	r = compressor_encode(&c, true, buf2, out_size2, &ret_done2);
        if (r < 0) {
                log_debug_errno(r, "compressor_decode failed: %m");
                return 0;
        }
